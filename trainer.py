...
import data_manager
import push_results
from active_inference_engine import compute_active_inference_scores
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (Active Inference Engine) ===")
        ...
        for win in config.WINDOWS:
            ...
            try:
                scores_df = compute_active_inference_scores(
                    prices   = prices,
                    macro_df = macro_df,
            ...
            for t, row in scores_df.iterrows():
                ...
                score_records[t] = {
                    ...
                    "belief_forecast":         float(row["belief_forecast"]),
                    "surprise_signal":         float(row["surprise_signal"]),
                    "belief_signal":           float(row["belief_signal"]),
                    "model_confidence":        float(row["model_confidence"]),
                    "prediction_error_today":  float(row["prediction_error_today"]),
                }
        ...
        top_etfs    = [
            {
                ...
                "belief_forecast": rec["belief_forecast"],
                "surprise_signal": rec["surprise_signal"],
                "belief_signal": rec["belief_signal"],
                "model_confidence": rec["model_confidence"],
                "prediction_error_today": rec["prediction_error_today"],
            }
        ...
        for win, score_records in window_results.items():
            ...
            windows_tab2[str(win)] = {
                ...
                "full_ranking": [
                    [t, rec["score"], rec["belief_forecast"], rec["surprise_signal"],
                     rec["belief_signal"], rec["model_confidence"]]
                    for t, rec in sw
                ],
            }
    ...
    tab1_path = Path(f"results/active_inference_engine_{today}.json")
    with open(tab1_path, "w") as f:
        json.dump(convert_to_serializable({"run_date": today, "universes": all_results}), f, indent=2)

    tab2_path = Path(f"results/active_inference_engine_windows_{today}.json")
    ...
    print(f"\n=== Active Inference Engine complete: {tab1_path.name}, {tab2_path.name} ===")
